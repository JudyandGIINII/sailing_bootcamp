...
> **현재 상태: 구현 전 기획/기술계획 완료.** 실제 앱 소스, 패키지 manifest, dependency, 테스트 런타임은 아직 생성하지 않았습니다.
...
결정론적 simulation core는 renderer/DOM/storage/network와 분리하며, PixiJS는 canonical simulation state를 변경하지 않는 projection adapter로만 사용합니다.
...
코드 bootstrap, dependency install, Git worktree 생성은 사용자 명시 **AP-0 구현 승인** 뒤에만 진행합니다. 구현 prototype은 가능하더라도 현재 모든 validation record가 `assumption`이므로 제품 release는 불가합니다.
