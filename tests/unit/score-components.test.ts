...
describe('L04 component derivation', () => {
  it('declares observation unavailable because nothing records it', () => {
    const observation = componentBy(computeComponents(cleanRaw, [])!, 'observation');
  ...
  it('credits ten judgment points per recorded correction and points at their events', () => {
    const judgment = componentBy(computeComponents(cleanRaw, [
  ...
  it('saturates judgment at the declared maximum', () => {
    ...
    expect(componentBy(computeComponents(cleanRaw, ledger)!, 'judgment').points).toBe(25);
  });
  ...
  it('gives full control-stability credit to a session that never reverses the helm', () => {
    expect(componentBy(computeComponents(cleanRaw, [])!, 'control_stability').points).toBe(25);
  });
  ...
  it('subtracts five control-stability points per reversal and floors at zero', () => {
    ...
    expect(componentBy(computeComponents(cleanRaw, oscillating)!, 'control_stability').points).toBe(0);
  });
  ...
  it('scores safety from the most severe recorded clearance level', () => {
    ...
    expect(componentBy(computeComponents(cleanRaw, [])!, 'safety').points).toBe(25);
    expect(componentBy(computeComponents(cautionRaw, [])!, 'safety').points).toBe(12);
    expect(componentBy(computeComponents(dangerRaw, [])!, 'safety').points).toBe(0);
  });

  it('points the safety component at the clearance episodes that caused it', () => {
    const cautionRaw = { lesson_id: 'L04', highest_clearance_alert: 'caution' } as unknown as RawSimulationState;
    const safety = componentBy(computeComponents(cautionRaw, [
  ...
  it('awards the goal only when a mark arrival was recorded', () => {
    expect(componentBy(computeComponents(cleanRaw, [])!, 'goal').points).toBe(0);
    const arrived = componentBy(computeComponents(cleanRaw, [
  ...
  it('returns all five components in a stable order', () => {
    expect(computeComponents(cleanRaw, [])!.map((component) => component.key))
