...
>
> **Current continuation boundary (2026-07-25):** The approved local-only Scenario 1 P4 D1–D10 scope is implemented as additive `scenario1-p4.html` and `src/scenario1/p4/*`. It provides a deterministic P2→P3 bridge and memory-only retry. Its `scenario1-p2-p3-binding-v1` discriminator is P4 in-memory only: it does not change Replay V2, IndexedDB, the legacy main entry, L01–L05, backend/network behavior, deployment, or access policy.
>
> **P4 scoring/verification boundary:** `sail_wind_fit_q` is the exact synthetic six-field contributor (main/jib deployment, main/jib trim, and main/jib reef); it makes no wind or physical inference. P4 accepts only canonical user inputs, derives bridge records in canonical order, and verifies them fail-closed. The UI explicitly states that it is synthetic/non-navigation and has no saved replay.
>
> **P4 QA and Git status:** Independent worktree QA passed `npm run typecheck`; focused 11 P4 unit tests; `npm test` (21 files / 226 tests); `npm run build`; `npm run test:smoke -- --grep 'dedicated P4 entry'` (1 passed); `git diff --check`; and browser-entry walkthrough/visual check. Commit `e87b3145ca777dcd84f616d4fbcfe39e497ab35e` has been fast-forward integrated to local `main` and pushed; `origin/main` contains it. No deployment was performed.
>
> **P5 source/integration state:** Source milestone `8f6677aff6d0e4ea729ea366d2aca96e1ab36878` (`feat(scenario1): add P5 memory-only debrief comparison`) was fast-forwarded into this fresh integration worktree from the verified P4 documentation baseline `d68266a...`. Its additive paths are `scenario1-p5.html`, `src/scenario1/p5/*`, dedicated tests, and one Vite multipage-input addition; P4/P2/P3/Replay V2/IndexedDB/the legacy entry/L01–L05 remain unchanged. P5 source/integration verification is complete.
>
> **P5 comparison and QA boundary:** At most two terminal attempts exist only in browser memory and clear on reload/navigation. P5 accepts only the fixed seed, exact P4 binding identity, and exact target; malformed/tampered history and numeric/target/control/trace mismatches fail closed. Terminal canonical values are deep immutable copies and differences are current minus previous. The UI is neutral previous/current/difference and changed/unchanged only, with no physical, safety, navigation, certification, or qualitative-improvement claim. Final controller QA passed `npm run typecheck`; `npm test` (**22 files / 232 tests**); `npm run build`; `npm run test:smoke -- --grep 'P4|P5'` (**3 passed**); `git diff --check`; independent read-only source review (**APPROVE**); manual baseline → retry → previous/current comparison walkthrough; and keyboard focus retention, AT-style non-pointer activation, 320px touch, focusable/keyboard-scrollable comparison regions, and visibility no-mutation proof. No deployment, release, or public-access action is included.
>
...
- **Current Scenario 1 P4 boundary:** the approved D1–D10 local first-playable candidate is implemented only as additive `scenario1-p4.html` and `src/scenario1/p4/*`. The deterministic P2→P3 bridge, canonical derived-record verification, and same-seed memory-only retry are P4-local. `scenario1-p2-p3-binding-v1` is not a Replay V2, IndexedDB, legacy-entry, L01–L05, backend/network, deployment, or access-policy change. P4 UI retains explicit synthetic/non-navigation and no-saved-replay boundaries.
- **Current P4 evidence and Git status:** independent worktree QA passed strict typecheck; focused 11 P4 unit tests; full 21-file / 226-test suite; production build; the dedicated-P4-entry smoke (1 passed); `git diff --check`; and browser walkthrough/visual check. Commit `e87b3145ca777dcd84f616d4fbcfe39e497ab35e` has been fast-forward integrated to local `main` and pushed; `origin/main` contains it. No deployment occurred; do not infer release or public-access change.
- **Current Scenario 1 P5 state:** `8f6677aff6d0e4ea729ea366d2aca96e1ab36878` is source/integration verified in this fresh integration worktree from `d68266a...`. The P5 comparison is browser-memory-only, capped at two terminal attempts, and fail-closed for any identity/history/value mismatch; it is not a P4/P2/P3/Replay V2/IndexedDB/legacy-entry/L01–L05 change. P5 source/integration verification is complete. No deployment, release, public-access, domain-validation, or real-world sailing meaning is implied.
