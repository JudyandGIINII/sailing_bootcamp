...
> **Handoff status:** AP-0A_IMPLEMENTED_IN_ISOLATED_WORKTREE_PENDING_CHECKPOINT_AND_AP-0B_USER_APPROVAL
...
AP-0는 release 권한이 아니며, 승인 메시지는 아래 ceiling 중 하나를 명시해야 한다. ceiling을 생략하면 **AP-0A만** 승인된 것으로 해석한다. 2026-07-19에는 AP-0A가 `/Users/hipgiinii/orca/workspaces/Sailing_training/sailing-ap0a-bootstrap-contract`에서 완료·독립 검증되었지만, source는 아직 uncommitted이고 shared `main`에 통합되지 않았다.
...
- 이 worktree의 source/lockfile은 uncommitted이므로 shared `main` 또는 release artifact로 취급하지 않는다.
- **다음 권한 경계:** user의 AP-0B 승인 없이는 Task 1–3 및 Task 2A/2B를 시작하지 않는다. AP-0B는 commit/push/release 권한을 포함하지 않는다.
