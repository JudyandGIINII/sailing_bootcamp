...
- [>] 2026-07-21 L03 bounded synthetic cue/checkpoint acknowledgment is under repair in an isolated worktree. `ALF-20260721-0855-l03-next-slice` permits only `pending → gust_wave_observed → complete/selected`; no wind/wave values, reef timing, advice, thresholds, safety, physics, navigation, scoring, or performance model. The uncommitted candidate must not merge/deploy until strict paused pre-terminal local save/load and full browser smoke pass.
...
6. L03 has a bounded approved plan but is currently an uncommitted repair candidate. Do not commit, integrate, or deploy it until controller QA and a fresh source review are green. L04–L05 or computed trim-effect proposals remain separately evidence-planned and must not infer physical/environment, registry, deployment, or release authority.
