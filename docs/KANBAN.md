...
7. Scenario 1 source work remains blocked only on a product decision, not an external gate: approve a versioned **synthetic** propulsion penalty curve/composition rule (or narrow the planned score to unavailable), then separately authorize a bounded isolated-worktree implementation scope. Do not infer a curve, default/reweight the 20% component, or treat this planning checkpoint as code authorization.
