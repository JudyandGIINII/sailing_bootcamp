...
import { l06ReplayBindings } from './content/l06-polar.js';
...
const lessons: readonly LessonRuntime[] = [
  ...
  { id: 'L04', bindings: l04ReplayBindings, actions: getLessonManifest('L04')!.permitted_actions, controls: 'Left/Right helm to steer toward the declared mark against the synthetic current; M/J cycle the declared main and jib trim; F toggles the declared reef; Space pause/resume.' },
...
mount.innerHTML = `
  <main>
    ...
    <fieldset id="lesson-fieldset"><legend>Training module selection</legend><label for="lesson-select">Lesson</label><select id="lesson-select"><option>L01</option><option>L02</option><option>L03</option><option>L04</option><option>L05</option><option>L06</option></select><p>Every lesson is an <strong>assumption</strong>; labels and text, not color alone, communicate that status.</p></fieldset>
    ...
    if (currentLesson.id === 'L01' && observation.status === 'declared_synthetic') {
      ...
      const polarText = (label: string, unit: string, value: number | 'declared-unavailable' | undefined) =>
        typeof value === 'number' ? `Synthetic computed ${label} ${numeric(value)} ${unit}.`
          : value === 'declared-unavailable' ? `Synthetic computed ${label} unavailable.`
